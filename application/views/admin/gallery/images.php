			...
			<div class="row sortable">
				<? foreach ($images as $image): ?>
					<div class="col-sm-6 col-md-4 col-xs-12 col-lg-3" id="id_<?= get_value($image, 'id') ?>">
						<div class="thumbnail">
							<a href="<?= $this->gallery_model->resizes_url . get_value($image, 'image') ?>"
							   class="colorbox" rel="gallery">
								<img src="<?= $this->gallery_model->thumbs_url . get_value($image, 'image') ?>"
									 alt="">
							</a>
							...
							<div class="caption">
								<p class="text-center">
									<a class="btn btn-sm btn-danger delete_image"
									   ...
									   data-id="<?= get_value($image, 'id') ?>"><span
											class="glyphicon glyphicon-remove"></span> Удалить</a>
									<a href="#"
									   data-image="<?= $this->gallery_model->resizes_url . get_value($image, 'image') ?>"
									   class="btn btn-sm btn-default edit_image"
									   data-id="<?= get_value($image, 'id') ?>"
									   data-url="<?= Settings::get('site_url') . 'admin/cropp_image/' . get_value($image, 'id') ?>"><span
